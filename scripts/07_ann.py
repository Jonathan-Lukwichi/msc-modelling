"""Plan §11.3 Step 6b: ANN (MLP) standalone on §3.4.3 consensus features.

Random search 20 iter per §3.5.9. Best by val MAPE; then rolling-origin
weekly refit on val with the winning architecture.
"""
...
from src.forecasting.models.ann import random_search_cv, rolling_forecast
...
def main() -> None:
    ...
    print("\nRandom search 20 trials via inner rolling-origin CV (10 folds)...")
    t0 = time.time()
    best, trace = random_search_cv(X_train, y_train, n_iter=20, n_folds=10, seed=42)
    ...
    print("\nRunning rolling-origin ANN forecast on val (weekly refit)...")
    t0 = time.time()
    ...
    out_pred = ROOT / "artefacts" / "predictions" / "ann.csv"
    out_metrics = ROOT / "artefacts" / "metrics" / "ann_metrics.csv"
    out_best = ROOT / "artefacts" / "models" / "ann_best_params.json"
    out_trace = ROOT / "artefacts" / "metrics" / "ann_hpo_trace.csv"
